...
// 주요 하우스는 당일 리포트가 없더라도, PDF 원문이 있으면 상단에 우선 노출한다.
// PDF 원문이 없는 하우스는 홈페이지로 연결하지 않고 카드에서 제외한다.
...
async function run() {
  ...
  // PDF 원문이 첨부된 항목만 대상으로 한다. (홈페이지·읽기페이지 링크는 제외)
  const pdfItems = items.filter((x) => isPdf(x.pdf));
  let pick = pdfItems.filter((x) => x.date === today);
  if (pick.length < 6) pick = pdfItems.slice(0, 16);        // 당일 물량 적으면 최근분

  // 주요 증권사 우선 → 상단. 당일 목록에 없으면 전체 목록의 최신 PDF 자료를 확인한다.
  const byHouse = new Map();
  for (const it of pick) if (!byHouse.has(it.house)) byHouse.set(it.house, it);
  const latestByHouse = new Map();
  for (const it of pdfItems) if (!latestByHouse.has(it.house)) latestByHouse.set(it.house, it);
  const uniq = [];
  ...
  const literal = uniq.map((it) =>
    `{house:'${esc(it.house)}',title:'${esc(it.title)}',date:'${esc(it.date)}',url:'${esc(it.pdf)}',sum:'${esc(it.sum || '')}',big:${it.big ? 'true' : 'false'}}`,
  ).join(',\n   ');
